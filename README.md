...
### Practical Adjustments to Your Script

To address the "lost in the middle" issue in your current script, consider implementing data chunking and refined prompts. Here's an adjusted version of your script incorporating these strategies:
...
if __name__ == "__main__":
    try:
        ...
        chunks = list(chunk_data(formatted_text, 100))
        complete_titles = []

        for index, chunk in enumerate(chunks, start=1):
            context = chunk
            prompt = (
                f"[INST]<<SYS>> RESPOND WITH THE LIST OF MOVIE TITLES BELOW - CHUNK {index} OF {len(chunks)}\n\n"
                f"[context]\n{context}\n"
                "\n[/context]<</SYS>> Please list all movie titles in this chunk without omissions.[/INST]"
            )
            ...
            if res.get('message') and res['message'].get('content'):
                titles = res['message']['content']
                complete_titles.append(titles)
            else:
                print(f"No response received for Chunk {index}.")

        # Combine all chunks
        all_titles = "\n".join(complete_titles)
        print("Complete List of Movie Titles:")
        print(all_titles)
...
- **Data Chunking**: The `chunk_data` function divides the 1,000 titles into 10 chunks of 100 titles each.
- **Refined Prompts**: Each prompt specifies the chunk number, guiding the model to focus on that specific subset.
- **Iterative Processing**: The script processes each chunk sequentially, aggregating the results to form the complete list.
...
**Happy Coding!****"Lost in the Middle": Pay More Attention To Attention**
